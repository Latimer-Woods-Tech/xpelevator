...
| ID | Requirement | Status | Verification |
|---|---|---|---|
| R-040 | Self-serve operator onboarding → operator workspace that creates + manages client orgs beneath it | roadmap | Phase 4; data-model + management-API foundation shipped as R-048; remaining: self-serve onboarding UX + white-label (R-044) on `@latimer-woods-tech/operator` |
...
| R-044 | White-label operator branding (name/logo/colors) + operator subdomain | building | branding data-model + admin management API shipped as R-049; remaining: operator subdomain + the client-facing render surface. §17.2 launch rung |
