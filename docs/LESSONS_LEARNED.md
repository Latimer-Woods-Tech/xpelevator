...
> Last updated: 2026-02-27 (sprint 11 — Telnyx STT payload field + DTMF-disable fixes)
...
| **`call.gather.ended` reads wrong STT transcript field** (BL-090) | Telnyx sends STT results as `payload.speech_results.transcription`, NOT as `payload.transcript`. Our TypeScript interface declared `transcript?: string` which is always `undefined` after a speech gather — causing every speech turn to silently fall through to "Are you still there?" | Update interface to include `speech_results?: { transcription?: string; results?: TelnyxSpeechResult[] }`. Read with `payload.speech_results?.transcription \|\| payload.speech_results?.results?.[0]?.transcript` | When integrating any webhook-driven STT API, check the payload structure with the provider's Inspector tool before writing the field read; never assume a `transcript` flat field exists — Telnyx nests it inside `speech_results` |
| **DTMF terminates STT gather before speech is captured** (BL-091) | `gather_using_speak` without `valid_digits: ''` responds to any phone key-press by ending the gather immediately in DTMF mode. The resulting `call.gather.ended` event has `digits` but no `speech_results`, so the transcript is empty and the call loops to "Are you still there?". Confirmed from Telnyx Inspector: user pressed `8`, gather ended via digit, no speech was recorded | Set `valid_digits: ''` in the `gather_using_speak` request body. An empty string tells Telnyx no digits are valid — only speech recognition can end the gather | Always set `valid_digits: ''` when issuing a speech-only gather; any other value (or omitting it) enables DTMF termination for all or specific keys |
