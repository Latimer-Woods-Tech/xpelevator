# XPElevator — Domain Reference

> **Scope:** app-specific *product/domain* knowledge — the MSP role ladder, the
> scenario catalog, evaluation criteria, difficulty escalation, the scoring
> rubric, the `Scenario.script` JSONB shape, and persona-prompt templates.
> This is training-content reference, **not** system architecture — the
> grounded system description lives in [`ARCHITECTURE.md`](./ARCHITECTURE.md)
> and the accountable requirements in [`REQUIREMENTS.md`](./REQUIREMENTS.md).
>
> Relocated verbatim out of `ARCHITECTURE.md` (2026-07-09) so the architecture
> spine stays short and machine-checkable per PLATFORM_STANDARDS §16. The stale
> "API Endpoint Reference" section that used to sit here was dropped — it listed
> routes that no longer exist (e.g. `/api/simulations/[sessionId]/message`); the
> live route table is in `ARCHITECTURE.md` → *Containers (Current)*.
